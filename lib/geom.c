...
float calc_abs_metric (const float *dot1, const float *dot2)
{
    int i;
    float res = 0;
    for (i=0; i<N; i++) res += fabsf (dot1[i] - dot2[i]);
    return res;
}
...
float calc_sqr_metric (const float *dot1, const float *dot2)
{
    int i;
    float res = 0;
    for (i=0; i<N; i++) res += powf (dot1[i] - dot2[i], 2.0);
    return res;
}
...
int dot_betweenp (const float *min, const float *max, const float *dot)
{
    ...
    for (i=0; i<N; i++) {if ((dot[i] < min[i]) || (dot[i] > max[i])) return 0;}
    return 1;
}
...
float* closest_in_set (float set[][N], int n, const float *dot, float (*metric) (const float*, const float*))
{
    float *res = set[0];
